...
public class FifthTask {
	...
	public String[] sameVowelGroup(String[] args) {
		...
		/*
		 * Алгоритм:
		 * 1. Нужно избавиться от всех согласных в первом слове - StringBuilder + массив-шаблон
		 * 2. Сначала нужно убрать все повторяющиеся гласные в первом слове - отправить в множество
		 * 3. Далее по одной букве проверять их наличие в каждом слове из массива
		 * 4. Сжать результирующий массив до нужных размеров, так как изначально взят максимальный размер.
		 */
	...
	public boolean validateCard(long card) {
		...
		// Переворачивание числа без последней цифры оригинала
		// Вообще есть метод reverse в классе Integer, можно и через него
		int[] mcard = new int[len - 1];
		long pseudoCard = card / 10;
		
		//System.out.println("Pseudo card = " + pseudoCard);
		//System.out.println("Mod 10 from pseudo = " + ((int)(pseudoCard % 10)) + " == " + (pseudoCard % 10));
		...
		//for (int num : mcard) System.out.print(num + " ");
		//System.out.println("");
		...
		for (int i = 0; i < mcard.length; i += 2) {
			...
			// Складывание цифр числа пока полученное число не станет цифрой
		...
		//System.out.println("massive after calculation");
		//for (int num : mcard) System.out.print(num + " ");
		//System.out.println("\n");
	...
	public String numToEng(int num) {
		// Если число будет однозначным, совпадене со словом 1 в 1
		String[] units = {
			"zero", "one", "two", "three", "four", "five", "six", "seven", "eight", "nine" };
		// Добавлено пустое множество, чтобы было легко создаваемое совпадение номера индекса и цифры в десятках
		String[] tens = {
		...
		// Если число двузначное или трёхзначеное
		if (num_str.length() == 2) {
	...
	public String correctTitle(String line) {
		// Может добавить обработку слов через тире?
	...
	public String hexLattice(int amount) {
		 ...
		 * 1. Нахождение элементов можно задать рекурентной формулой вида: Y(n+1) = 6 * (n-1) + Y(n), где
		 * 		+ Y(n) - текущий элемент, 
		 * 		+ 6 - самый первый шаг (переход от 1 элемнта к 7 элементам)
		 * 		+ Y(1) = 1;
		 * 2. С каждым последующим увеличением размеров фигуры шаг увеличивfется на 6 (равномерно)
		 * 3. Количество точек в верхней линии равняется порядковому номеру фигуры:
		 * 		Номер = 1, кол-во точек = 1, размеры фигуры 1 на 1
		 * 		Номер = 2, кол-во точек = 7, размеры фигуры 3 на 3
		 * 		Номер = 3, кол-во точек = 19, размер фигуры 5 на 5
		 * 		и т.д.
		 * 4. Количество точек по горизонтали (ОХ) и вертикали (ОУ) равны, и вычисляются по формуле:
		 * 		OX(n) = OY(n) = OX(n - 1) + 2;
		 * 
		 * 	Статистика изменения порядкового номера (n), размеров фигуры (OX, OY) и общего количества точек (amount):
		 * n:		1	2	3	4
		 * OX(OY):	1	3	5	7
		 * amount:	1	7	19	37
		 */
		...
		//System.out.println("amount = " + amount + ", count = " + count + ", lenX = " + lenX);
		
		/*
		 * Пояснения:
		 * Отрисовка начинается с вершины фигуры.
		 * 1. Заполнить плюсами начало строки по формуле: макс_кол-во_в_строке - текущее_кол-во_в_строке
		 * 2. Добавить знак "o" и пробел после него.
		 * 3. Увеличить количество знаков в строке на значение "operation", которое = 1 
		 * при проходе от вершины до середины, и = -1 при проходе от середины до низа фигуры.
		 * 4. Проверить, если макс_кол-во_в_строке == текущее_кол-во_в_строке, то изменить операцию приращения на 1 на 
		 * операцию понижения на 1 (операция применяется после создания каждой строки)
		 * 5. Заменить последний пробел в строке на знак переноса строки
		 */
		StringBuilder build = new StringBuilder();
		...
		for (int i = 0; i < lenY; i++) {
			...
			// Заполнение начала строки пробелами
